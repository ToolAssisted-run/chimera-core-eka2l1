#!/bin/sh
# Applies the numbered patches to the extern/eka2l1 submodule. Idempotent: a tree
...
root="$(cd "$here/.." && pwd)"
# EKA2L1_TREE points it at another checkout: how this script is tested without
# touching the tree a build depends on
tree="${EKA2L1_TREE:-$root/extern/eka2l1}"
...
if [ "$(git -C "$tree" rev-parse --show-toplevel 2>/dev/null)" != "$(cd "$tree" 2>/dev/null && pwd -P)" ]; then
	...
	echo "  git -C $root submodule update --init --recursive extern/eka2l1" >&2
	exit 1
fi
...
if [ "$wrong" -ne 0 ]; then
	echo "extern/eka2l1 is partly patched. To start again from the submodule's HEAD:" >&2
	echo "  git -C extern/eka2l1 reset --hard && git -C extern/eka2l1 clean -fd && waterbox/apply-patches.sh" >&2
