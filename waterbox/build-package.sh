#!/bin/sh
# Builds the EKA2L1 waterbox core package and installs it into a chimera
# checkout as build/Cores/eka2l1.chimeraCore.
#
...
eka2l1_pin="$(git -C "$root/extern/eka2l1" describe --tags --always 2>/dev/null || echo unknown)"
...
json.dump({
    ...
    "upstream": {"name": "EKA2L1", "pin": "$eka2l1_pin"},
    "builtOn": "$os_id",
...
mkdir -p "$cores_dir"
zip_path="$cores_dir/eka2l1.chimeraCore"
rm -f "$zip_path"
...
for cache in "$chimera_root"/build/CoreCache/eka2l1-*; do
